...
/** Step 1: audio → transcription text (markdown string). */
export const SCRIBE_TRANSCRIBE_PATH = "/api/audio/transcribe";
/** @deprecated Backend issues — use SCRIBE_TRANSCRIBE_PATH + report/from-text instead. */
...
function logScribeStepResponse(stepLabel, path, url, status, headers, body, requestMeta = {}) {
  ...
  if (stepLabel.includes("Step 1")) {
    console.info("SCRIBE STEP 1 RESPONSE (transcribe):", body);
...
/**
 * Step 1 — audio → transcription text (`/api/audio/transcribe`, markdown string).
...
export async function transcribeAudio(file, opts = {}) {
  ...
  return requestScribe(
    SCRIBE_TRANSCRIBE_PATH,
    { method: "POST", body },
    {
      ...opts,
      stepLabel: "Step 1 — transcribe response",
      requestMeta: {
        fileName: safeName,
        mimeType: mime,
...
/**
 * @deprecated Use transcribeAudio — transcribe-and-report has known backend issues.
 * @param {Blob|File} file
 * @param {object} [opts]
 */
...
export async function transcribeAudioAndGenerateReport(file, opts = {}) {
  ...
  const audioData = await transcribeAudio(file, opts);
